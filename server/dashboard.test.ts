...
describe("dashboard control-plane contracts", () => {
  ...
  it("creates a queued run with isolated resources", () => {
    const input = testRunInputSchema.parse({
      risk: "CRITICAL",
      parallel: true,
  ...
  it("supports the required report formats", () => {
    expect(
      ["HTML", "JSON", "JUnit", "SARIF"].map(value =>
        reportFormatSchema.parse(value)
      )
    ).toEqual(["HTML", "JSON", "JUnit", "SARIF"]);
  });
});
...
describe("dashboard procedures", () => {
  it("returns the control-plane snapshot with score and required metrics", async () => {
    ...
    expect(snapshot.metrics.map(metric => metric.label)).toContain(
  ...
  it("queues a run through the typed procedure with isolated chain state", async () => {
    const run = await caller().dashboard.createRun({
      risk: "HIGH",
      parallel: false,
      profile: "adversarial",
    });
...
it("saves an authenticated suite configuration", async () => {
  const result = await caller().dashboard.configureSuite({
    suiteName: "Smart Contract",
    enabled: true,
    profile: "critical-core",
  });
...
describe("nightly report ingestion", () => {
  it("maps SARIF results and JUnit failures into dashboard finding records", () => {
            ...
            results: [
              {
                ...
                locations: [
                  {
                    physicalLocation: {
                      artifactLocation: { uri: "SimpleProxy" },
                    },
                  },
                ],
              },
    ...
    const junit = parseJunit(
      '<testsuite><testcase name="blocked upgrade" classname="SimpleProxy"><failure message="blocked" /></testcase></testsuite>'
    );
    expect(junit[0]).toMatchObject({
      severity: "HIGH",
      category: "CI integration failure",
      component: "SimpleProxy",
    });
  });
});
...
describe("protected proxy governance procedure", () => {
  const governanceInput = {
    proxyAddress: `0xproxy-${Date.now()}`,
    caller: "0xattacker",
    admin: "0xadmin",
    implementation: "0ximpl-blocked",
    allowlisted: false,
  };
  ...
  it("rejects unauthenticated callers before governance evaluation", async () => {
    ...
    await expect(
      unauthenticated.dashboard.enforceProxyUpgrade(governanceInput)
    ).rejects.toMatchObject({ code: "UNAUTHORIZED" });
  });

  it("persists a critical finding when an authenticated upgrade is blocked", async () => {
    const result =
      await caller().dashboard.enforceProxyUpgrade(governanceInput);
    expect(result).toMatchObject({
      allowed: false,
  ...
  it("persists a critical finding for an authenticated non-allowlisted implementation", async () => {
    const result = await caller().dashboard.enforceProxyUpgrade({
      ...governanceInput,
      proxyAddress: `0xproxy-${Date.now()}-allowlist`,
      caller: "0xadmin",
      allowlisted: false,
    });
    expect(result).toMatchObject({
      allowed: false,
      reason: "IMPLEMENTATION_NOT_ALLOWLISTED",
    });
    expect(result.finding?.actualBehavior).toContain(
      "IMPLEMENTATION_NOT_ALLOWLISTED"
    );
    expect(result.finding?.locations?.[0]).toMatchObject({
      physicalLocation: {
        artifactLocation: { uri: "automation/fixtures/UpgradeableProxy.sol" },
...
const adminRetentionCaller = () =>
  appRouter.createCaller({
    user: {
      id: 2,
      openId: "admin-retention-test",
...
describe("report retention controls", () => {
  ...
  it("allows administrators to delete a report history record with a retention audit contract", async () => {
    const result = await adminCaller().dashboard.deleteReportHistory({
      runId: "RUN-RETENTION-001",
      format: "SARIF",
    });
    expect(result).toMatchObject({
      runId: "RUN-RETENTION-001",
  ...
  it("denies report deletion to non-administrator operators", async () => {
    await expect(
      caller().dashboard.deleteReportHistory({
        runId: "RUN-RETENTION-002",
        format: "JUnit",
      })
